...
- `required`: 단순 프로필·소속·장비 같은 구체적 사실 질문인데 로컬 `lore_reference`에 충분한
  `world_fact`가 없는 경우
- `auto`: 로컬 world_fact가 충분한 단순 사실 질문과 그 외 일반 답변. 필요하면 모델이 검색함
...
4. 마지막에 현재 관계나 의미를 과장하지 않고 짧게 정리할 수 있음

특히 관계·사건 질문은 한 검색 결과에 적힌 결론만 찾지 않습니다. 관련 사건, 각 인물의 역할,
서로에 대한 언급, 같은 사건에 참여한 기록 등을 여러 자료에서 찾아 연결할 수 있습니다. 다만
같은 작전에 참여했다는 사실만으로 두 사람이 직접 만나거나 대화했다고 단정하지 않습니다.
명시적인 대면 횟수나 대화 내용이 확인되지 않으면 그 부분은 확인되지 않는다고 남겨 둡니다.
