...
채택 목표는 한국어 Discord RP에서 안정적으로 사용할 수 있는 리오 관련 공식 공개 정보 전체입니다.
공식 자료, 커뮤니티 위키, 비공식 미러는 출처 성격을 구분하고, 일본 서버 선행 내용·유출·검증되지
않은 팬 해석은 공식 사실로 승인하지 않습니다.

## 커버리지 상태

현재 저장소에는 웹에서 확인 가능한 공식 공지와 교차 확인 가능한 공개 자료를 바탕으로 한 핵심
lore만 들어 있습니다. 공식 전체 커버리지를 완성하려면 인게임에서 열람 가능한 다음 원문을 별도
자료로 확보해 `rio-lore` 검수 파이프라인에 넣어야 합니다.

- 기본 리오와 리오(무장)의 학생 프로필, 스킬, 고유무기, 로비/메모리얼/카페/스케줄/보이스
- 메인 스토리 Vol.2, Final, Ex. 데카그라마톤 편의 리오 등장 장면
- Code: BOX / 밀레니엄 EXPO 과외조사 전체 에피소드
- 강철대륙 공략전 및 관련 연합 작전/Afterword
- 리오가 직접 등장하거나 언급되는 공식 이벤트, 공지, PV, 학생 소개 자료

자료가 확보되지 않은 영역에 대해서는 봇이 “내가 전부 알고 있다”고 단정하지 않도록, 확인된 항목만
짧은 world fact 또는 interpretation으로 승인합니다.
...
- Code: BOX / 밀레니엄 EXPO 과외조사에서 세이아·네루와 협력한 사건의 핵심
- Ex. 데카그라마톤 편 제3장과 강철대륙 공략전에서 리오(무장)가 등장하는 최신 축
