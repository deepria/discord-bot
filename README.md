...
현재 대화 중인 서버의 사용 가능한 커스텀 이모지를 자동으로 조회해 답변에 사용할 수 있어요.
정적 이모지는 `<:이름:ID>`, 애니메이션은 `<a:이름:ID>` 표기를 사용하며, 모델에는 이름과
정확한 표기를 전달해요. `hina`로 시작하는 이름을 우선하고 이름·ID 순으로 최대 24개를
선정해요. 많은 이모지 중 의미 검색으로 선택하는 기능은 아직 없어요.
역할 제한, 비활성화 여부를 확인하고 생성 후에도 목록을 재확인해요. 목록 밖 ID는 제거하고
이름·애니메이션 표기를 정규화하며 답변당 최대 2개로 제한해요. 일반 유니코드 이모지는 유지해요.

별도 이미지 분석 API는 호출하지 않아요. `hina_happy`, `hina_tired`처럼 이름에 의미가
드러나면 더 적절히 선택할 수 있어요. 실제 그림의 의미를 자동으로 이해하지는 못해요.
서버의 공개 여부와 무관하게 봇이 해당 서버에 있어야 목록에 접근할 수 있어요.
이 버전은 외부 서버 이모지를 가져오거나 서버 이모지를 DM에서 사용하는 기능은 포함하지 않아요.
그 경우에는 별도 이모지 공급 서버와 외부 이모지 권한 처리가 필요해요.
이모지 목록은 캐시에서 읽고 Gateway의 갱신을 사용하며, 사용할 이모지를 매번 API로 재등록하지 않아요.

[Discord 이모지 문서](https://docs.discord.com/developers/resources/emoji)에 역할별 사용 제한과
사용 가능 상태가 정의되어 있어요. 실제 서버에서의 렌더링은 봇 토큰 설정 후 확인해야 해요.
...
**총 34개를 통과**했고, Ruff 검사와 Python 문법 컴파일을 확인했어요.
