...
from rio_bot.config import parse_call_prefixes
from rio_bot.routing import Scope, chunks, trigger_text
...
class SharedContextTests(unittest.TestCase):
    ...
    def test_recent_ttl_and_capacity(self):
        ...
        with patch("rio_bot.recent.time.monotonic", return_value=10):
            recent.add(Scope(1, 10, 100), 1, "A", "old")
            recent.add(Scope(1, 20, 100), 2, "A", "new")
        self.assertEqual(len(recent.buffers), 1)
        with patch("rio_bot.recent.time.monotonic", return_value=16):
