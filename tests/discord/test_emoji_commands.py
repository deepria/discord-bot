...
from rio_bot.emoji_commands import EmojiCommands, EmojiRegistry
...
class RegistryTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_source_registration_and_permissions(self):
        await self.registry.add("rio_happy", "기쁠 때", source="<:리오웃음:1234>")
        self.client.create_application_emoji.assert_not_awaited()
        same = NS(guild=NS(id=100, me=NS()))
        self.assertEqual((await self.registry.catalog(same))[0]["name"], "rio_happy")
    ...
    async def test_edit_remove_does_not_delete_source(self):
        await self.registry.add("rio_happy", "기쁨", source="1234")
        await self.registry.edit("rio_happy", "칭찬받았을 때")
        self.assertEqual(self.store.emoji_rows()[0]["description"], "칭찬받았을 때")
        await self.registry.remove("rio_happy")
    ...
    async def test_image_upload_validation(self):
        ...
        await self.registry.add("rio_blush", "부끄러움", attachment)
        ...
        with self.assertRaises(ValueError):
            await self.registry.add("rio_bad", "bad", attachment)
    ...
    async def test_admin_authorization_and_message_crud(self):
        ...
        self.assertIn("관리자", await group.handle(message, "등록 rio_happy 1234 기쁠 때"))
        self.assertEqual(self.store.emoji_rows(), [])
        message.author.id = 100
        self.assertIn("등록했어요", await group.handle(message, "등록 rio_happy 1234 기쁠 때"))
        self.assertIn("기쁠 때", await group.handle(message, "목록"))
        await group.handle(message, "수정 rio_happy 칭찬받았을 때")
        self.assertEqual(self.store.emoji_rows()[0]["description"], "칭찬받았을 때")
        await group.handle(message, "삭제 rio_happy")
        self.assertEqual(self.store.emoji_rows(), [])
    ...
    async def test_message_image_and_invalid_arguments(self):
        ...
        self.assertIn("동시에", await group.handle(message, "등록 rio_happy <:x:1234> happy"))
        attachment.read.assert_not_awaited()
        self.assertIn("등록했어요", await group.handle(message, "등록 rio_happy 기쁠 때"))
        self.assertEqual(await group.handle(message, "등록"), group.HELP)

    async def test_concurrent_duplicate_registration(self):
        import asyncio
        results = await asyncio.gather(
            self.registry.add("rio_happy", "happy", source="1234"),
            self.registry.add("rio_happy", "happy", source="1234"), return_exceptions=True)
    ...
    async def test_registry_persistence(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            one = Store(path)
            one.add_emoji("rio_happy", "1234", "happy", "100")
            one.close()
            two = Store(path)
