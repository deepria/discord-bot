...
async def collect_visual_inputs(
    ...
    *,
    ...
    async def add_bytes(data: bytes, source: str, name: str, metadata: dict[str, str]):
        ...
        if mime is None:
            return
        result.append(VisualInput(data=data, mime_type=mime, source=source, name=name, **metadata))
        counts[source] += 1
        used += len(data)
    ...
    if include_reply:
        ...
        if target is not None and (
            allowed_reply_author_id is None or target_author_id == allowed_reply_author_id
        ):
            await collect_message(
                target,
