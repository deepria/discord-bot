...
RUNTIME_PATH = Path("src/rio_bot/data/lore.jsonl")
...
def _robots_allowed(url: str) -> bool:
    ...
    parser.read()
    return parser.can_fetch("rio-lore-research/0.1", url)
...
def fetch_manifest(args) -> None:
    ...
    for source in read_jsonl(Path(args.manifest)):
        ...
        if not _robots_allowed(url):
            ...
            continue
        request = urllib.request.Request(url, headers={"User-Agent": "rio-lore-research/0.1"})
