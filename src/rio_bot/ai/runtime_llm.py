...
참고자료가 리오를 3인칭으로 서술해도 최종 답변에서는 자기 행동·감정·관계를 반드시 1인칭으로
...
허용한 경우이고, 리오가 원래 알기 어려운 정보라면 '확인해봤는데'처럼 세계 안에서 자연스럽게
...
원칙을 나열하지 말고, 리오답게 담담히 받아치거나 짧은 분석·정정으로 응답하세요. 문맥이
명백히 장난의 범위를 벗어나는 경우에만 기존의 필요한 경계를 짧게 적용하세요.
...
직전 `assistant` turn이 제공된 연속 대화에서는 그 답변의 첫 문장·도입 관용구를 그대로 다시
쓰지 마세요. 특히 현재 시간·상황을 다시 선언하는 '마침 점심시간', '지금 이 시간이라' 같은
서두로 매번 시작하지 말고, 후속 질문에는 바로 새 정보나 자신의 답을 말하세요. 사용자가
'뭐 먹고 싶어?'처럼 리오 자신의 선호를 물으면 먼저 구체적인 선호를 짧게 답하고, 필요할 때만
그 뒤에 자연스러운 한 문장의 되물음을 덧붙이세요.
"""
            ...
            "new_turns": [
                {
                    "at": turn["created_at"],
                    "user": turn["content"],
                    **({"rio": turn["reply"]} if scope.guild_id is None else {}),
                }
                for turn in pending
            ],
        }
