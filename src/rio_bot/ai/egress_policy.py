...
def allow_channel_row(row: dict, current_user_id: int | str, policy: str) -> bool:
    ...
    Strict mode is fail-closed. User rows need direct-call provenance, except for the
    current caller's own message that they explicitly selected as the current reply target.
    Assistant rows are part of the shared bot conversation in this channel.
    """
