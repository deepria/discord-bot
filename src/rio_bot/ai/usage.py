...
def _chat_web_search_enabled() -> bool:
    ...
    if value in {"0", "false", "no", "off"}:
        return False
    logging.getLogger("rio").warning("Invalid CHAT_WEB_SEARCH value; web search disabled")
    return False
    ...
    def _emit(handler, row: dict):
        ...
        try:
            handler.handle(logging.LogRecord(
                "rio.usage", logging.INFO, "", 0,
                json.dumps(row, ensure_ascii=False), (), None))
        except OSError:
            logging.getLogger("rio").warning("Usage log write failed")
    ...
    async def request(self, client, operation: str, **kwargs):
        ...
        try:
            ...
            for item in responses:
                error_codes.extend(getattr(item, "_rio_error_codes", None) or [])
            if completed_empty:
